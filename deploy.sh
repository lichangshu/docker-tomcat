...
/usr/bin/docker build . -t $DOCKER_NAME:$TAG_NAME --build-arg WAR_FILE=app.war --build-arg CONTEXT_PATH=$CONTEXT_PATH --build-arg PORT=$PORT
...
/usr/bin/docker run -d --net host --restart always \
                         --name $DOCKER_NAME \
                         -e PORT=$4 \
